...
async function main() {
  ...
  // Filter to today
  const todayProd   = allProdEntries .filter(e => (e.date||'').slice(0,10) === today);
  const todayMat    = allMatPurchases.filter(p => (p.date||'').slice(0,10) === today);
  const todayPays   = allOrderPayments.filter(p => (p.date||'').slice(0,10) === today);
  const todayLabor  = allLaborPayments.filter(p => (p.date||'').slice(0,10) === today);
  const todayExp    = allExpenses.filter(e => (e.date||'').slice(0,10) === today);
...
<div style="background:#fff;padding:24px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
  ...
  <p style="color:#9ca3af;font-size:11px;margin-top:20px;text-align:center">
    ...
    Auto-generated by Urbanmud · ${fmtDate(today)}
  </p>
</div>
</body></html>`;
  ...
  await transporter.sendMail({
    ...
    to: allEmails.join(', '),
    subject: `Urbanmud Daily Report — ${fmtDate(today)}`,
    html,
    attachments: [{
      filename: `Urbanmud_Transactions_${today}.xlsx`,
      content: excelBuffer,
